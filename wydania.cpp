...
Wydania::Wydania(QWidget *parent) :
    QWidget(parent)
{
...
void Wydania::closeEvent(QCloseEvent *event)
{
    writeSettings();
    emit sig_close();
    event->accept();
...
void Wydania::createWidgets()
{
    ...
    model->setFilter("RodzajObrotu = 'Wydanie'");
    model->select();
...
void Wydania::createConnections()
{
...
void Wydania::slot_Close()
{
    parentWidget()->close();
}

void Wydania::slot_view_doubleClicked(const QModelIndex &index)
{
    int id = proxy->index(index.row(),0).data().toInt();
    emit sig_open(id);
}

void Wydania::slot_New()
{
    int id = 0;
    emit sig_open(id);
}

void Wydania::slot_Edit()
{
