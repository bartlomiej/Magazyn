...
Wydanie::Wydanie(int &_id, QWidget *parent)
...
void Wydanie::loadData()
{
    ...
    {
        leNumer->setText("");
        leRodzajObrotu->setText("Wydanie");
...
void Wydanie::closeEvent(QCloseEvent *event)
{
    if (okToClearData())
    {
        writeSettings();
...
void Wydanie::createConnections()
{
...
void Wydanie::slot_New()
{
    id = 0;
    loadData();
}

void Wydanie::slot_Close()
{
    parentWidget()->close();
}

void Wydanie::slot_Delete()
{
    if (id > 0)
    {
...
void Wydanie::slot_PozycjeDel()
{
    model->removeRow(view->currentIndex().row());
}

bool Wydanie::checkRequiredFields()
{
    bool ret = true;
